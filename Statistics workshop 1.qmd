...
<br> **1. Download the Excel file `1-sample cholesterol.xlsx` from Canvas.**
...
<br> **1. Download the Excel file `2-sample cholesterol.xlsx`**
...
```
1. n = the number of reps in each group; d = Cohen's d<br>
If there is an unequal number in groups, use pwr.t2n.test:<br> 
...
If there are unequal numbers in the groups, use pwr.t2n.test:<br> 
`pwr.t2n.test(n1 = NULL, n2= NULL, d = NULL, sig.level = 0.05, power = NULL)` where n1 = the number of data in group 1 and n2 = the number of data in group 2.
:::
...
`1.` **Download the Excel file *Paired sample cholesterol.xlsx* from Canvas.**
...
*NB* The first column identifies each subject.
:::
<br>
...
<br>
<br>`3.` **Explore the data** <br>`3a.` **Descriptive statistics**
...
These calculations use the 'long' data, but tell R that the data is paired, so the calculation is performed on the differences between the 2 groups.<br><br>
'6a.' **Wilcoxon signed rank test**
