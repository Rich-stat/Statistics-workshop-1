...
*NB* If you get a message saying that a packages does not exist, you need to install it first with `install.packages("name_of_package")`<br> If you get an error message saying that the `coin` package needs to be installed, install it with `install.packages("coin")`
:::
...
<br> **1. Download the Excel file** [1-sample cholesterol.xlsx](https://kingstonuniversity-my.sharepoint.com/:x:/g/personal/ku11417_kingston_ac_uk/IQAJNKa1AQh7Sq8KeI_mpseoAds51g-XCgtXBl8mFTZxqsg?e=R6aLq6) from Canvas to your Downloads folder.
...
<br> `3.3` The Shapiro-Wilk test for the normal distribution of data
...
#| eval: false
chol_1Sample |>
  rstatix::wilcox_test(LDL ~ 1, mu = 120)  #<1>
```

1.  Wilcoxon test for 1 variable, comparing the mean LDL against an expected value of 120. (`rstatix::` specifies to use the rstatix package)
...
#| eval: true
chol_1Sample |>
  rstatix::wilcox_test(LDL ~ 1, mu = 120) |>
  kable(align = 'l')
```
...
<br> **1. Download the Excel file** [2-sample cholesterol.xlsx](https://kingstonuniversity-my.sharepoint.com/:x:/g/personal/ku11417_kingston_ac_uk/IQDzlJTxbGx6RKTXK7uY-uiOAWXauJQagQJB2PImD7y63vg?e=xOiIep) from Canvas to your Downloads folder.
...
<br> `5c.` **Power analysis** <br> `5c.i` If there are data with equal numbers of replicates in each group:
...
1.  n = the number of reps in each group; d = Cohen's d

*NB* In this example there are 20 measurements in both groups, and we have calculated the effect size (d) as 1.05
...
n = the number of reps in each group; d = Cohen's d effect size<br><br>
:::

`5c.ii` If there are data with unequal numbers in the two groups:

```{r}
#| label: power_ind2
#| echo: TRUE
#| eval: false
pwr.t2n.test(n1 = NULL, n2= NULL, d = NULL, sig.level = 0.05, power = NULL) #<1>
```

1.  n1 = the number of data in group 1 and n2 = the number of data in group 2, d = Cohen's d effect size.
...
<br> `6c.` **Power analysis**<br> `6c.i` If there are data with equal numbers of replicates in each group:
...
n = the number of reps in each group; d = Cohen's d effect size
:::

`6c.ii` If there are data with unequal numbers in the two groups:
...
wilcoxon <- chol_2_ind_sample |>
  rstatix::wilcox_test(Cholesterol ~ Group) |>  #<1>
  add_significance()
wilcoxon
```

1.  Sends dataframe to wilcoxon test: Cholesterol grouped by Group. Stores result in an object called `wilcoxon`.(`rstatix::` specifies to use the rstatix package)
...
1.  Sends dataframe to the wilcoxon effect size function: groups Cholesterol data by Group
...
`1.` **Download the Excel file** [Paired sample cholesterol.xlsx](https://kingstonuniversity-my.sharepoint.com/:x:/g/personal/ku11417_kingston_ac_uk/IQC1xQBWa3EeT4agX4lARpRbAaUFuU7XM790KE9vOEqBdfM?e=8Y0DnA) from Canvas to your Downloads folder.
...
#| eval: false
pair_chol_long |>
  rstatix::wilcox_test(Cholesterol ~ Before_After, paired = TRUE) |> #<1>
  add_significance()
```

1.  Does the test on the Cholesterol data grouped by the Before_After variable, and says that it is a paired test. (`rstatix::` specifies to use the rstatix package)
...
#| eval: true
pair_chol_long |>
  rstatix::wilcox_test(Cholesterol ~ Before_After, paired = TRUE) |> 
  add_significance() |>
  kable(align = 'l')
```
